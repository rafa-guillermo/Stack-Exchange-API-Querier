...
function get() {
  var params = "questions?pagesize=100&fromdate=1220227200&todate=1594598400&order=desc&sort=activity&tagged=google-api&site=stackoverflow&";
  ...
  do {
    try {           
      var response = JSON.parse(UrlFetchApp.fetch(baseUrl + params + "key=" + STACKEXCHANGE_API_KEY + "&page=" + parseInt(count)).getContentText());
